...
class shader_prog (win: GlWindow.t) = object(self)
  ...
  val indexData = Bigarray.Array1.of_array Bigarray.int32 Bigarray.c_layout Data.indices

  val vaoObject1 = TutCommon.get_int @@ Gl.gen_vertex_arrays 1
  val vaoObject2 = TutCommon.get_int @@ Gl.gen_vertex_arrays 1
  ...
  method private initializeVertexArrayObjects () =
    ...
    Gl.bind_vertex_array vaoObject1;
    ...
    Gl.bind_vertex_array vaoObject2;
    
    let posDataOffset =
      float_size * 3 * (numberOfVertices / 2)
    and colorDataOffset =
      colorDataOffset + float_size * 4 * (numberOfVertices / 2) in

    (* Use the same buffer object previously bound to GL_ARRAY_BUFFER. *)
    Gl.enable_vertex_attrib_array 0;
    Gl.enable_vertex_attrib_array 1;
    Gl.vertex_attrib_pointer 0 3 Gl.float false 0 (`Offset posDataOffset);
    Gl.vertex_attrib_pointer 1 4 Gl.float false 0 (`Offset colorDataOffset);
    Gl.bind_buffer Gl.element_array_buffer indexBufferObject
  ...
  method draw () =
    ...
    Gl.bind_vertex_array vaoObject1;
    Gl.uniform_3f offsetUniform 0.0 0.0 0.0;
    Gl.draw_elements Gl.triangles numberOfIndices Gl.unsigned_int (`Offset 0);
