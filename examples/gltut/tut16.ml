...
class shader_prog (win: GlWindow.t) = object(self)
  ...
  method draw () =
    ...
    Gl.uniform_3f offsetUniform 0.0 0.0 (0.5);
  ...
  val mutable bDepthClampingActive = false

  method toggle_depth_clamping () =
    bDepthClampingActive <-
      match bDepthClampingActive with
        true -> Gl.disable Gl.depth_clamp; false
      | false -> Gl.enable Gl.depth_clamp; true
...
class renderer win w h = object(self)
  ...
  method keyboard_cb win key (w:int) (h:int) =
    match key with
      | `Escape -> self#quit ()
      | `Space  -> prog#toggle_depth_clamping ()
      | _ -> ()
