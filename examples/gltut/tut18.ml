...
module Mat4 = struct
  ...
  let make () =
    Array.init 4 (fun _ -> Array.make 4 0.0)
...
let vec3 (x,y,z) : vec4 = [| x;y;z;1.0|]

let stationaryOffset fElapsedTime =
  vec3 (0.0, 0.0, -20.0)


let ovalOffset fElapsedTime =
  let fLoopDuration = 3.0 in
  let fScale = 3.14159 *. 2.0 /. fLoopDuration in
  let fCurrTimeThroughLoop = mod_float fElapsedTime fLoopDuration in
  vec3(cos (fCurrTimeThroughLoop *. fScale) *. 4.,
       sin (fCurrTimeThroughLoop *. fScale) *. 6.,
       -20.0)


let bottomCircleOffset fElapsedTime =
  let fLoopDuration = 12.0 in
  let fScale = 3.14159 *. 2.0 /. fLoopDuration in
  let fCurrTimeThroughLoop = mod_float fElapsedTime fLoopDuration in

  vec3(cos (fCurrTimeThroughLoop *. fScale) *. 5.,
       -3.5,
       sin (fCurrTimeThroughLoop *. fScale) *. 5. -. 20.0)


class instance (f : float -> vec4)  = object

  method construct_matrix fElapsedTime =
    let v = f fElapsedTime in
    let mat = Mat4.identity () in
    Mat4.setcol mat 3 v;
    mat
